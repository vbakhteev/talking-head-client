...
import { LipsyncRu } from './lipsync-ru.mjs';
    ...
    if (this.speechQueue.length) {
      ...
      if (line.emoji) {
        ...
        try {
          ...
          const res = await fetch(this.opt.ttsEndpoint + "?" + new URLSearchParams(queryParams), o);


          if (res.status === 200) {

            // Audio data
            const audio = await this.audioCtx.decodeAudioData(await res.arrayBuffer());
            const audioDuration = audio.duration;
            const markTimeSec = audioDuration / marks / 1000;
            const data = {
              timepoints: Array.from({ length: marks }, (_, i) => { return { markName: i + 1, timeSeconds: (i + 1) * markTimeSec } })
            }
            this.speakWithHands();
